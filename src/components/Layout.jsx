    ...
    <div style={{ fontFamily: "'Inter', sans-serif", minHeight: "100vh", background: "#fafaf9" }}>
      <header style={{ background: "#1e3a5f", color: "white", padding: "0 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
          <Link to="/" style={{ color: "white", textDecoration: "none", fontWeight: 700, fontSize: 18, letterSpacing: "-0.3px" }}>
            ExpatAutoAdviser
          </Link>
          <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
            {cityLabel && (
              <span style={{ background: accentColor, padding: "4px 12px", borderRadius: 4, fontSize: 13, fontWeight: 600 }}>
                {cityLabel}
              </span>
            )}
        ...
        <nav style={{ background: "white", borderBottom: "1px solid #e5e7eb", overflowX: "auto" }}>
          <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", gap: 0, padding: "0 24px" }}>
            {navLinks.map(({ to, label }) => {
              ...
              return (
                <Link key={to} to={to} style={{ padding: "12px 14px", textDecoration: "none", fontSize: 13, fontWeight: isActive ? 600 : 400, color: isActive ? accentColor : "#374151", borderBottom: isActive ? `2px solid ${accentColor}` : "2px solid transparent", whiteSpace: "nowrap", transition: "color 0.15s" }}>
                  {label}
                </Link>
              );
            })}
