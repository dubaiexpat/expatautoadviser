...
const SEGMENTS = [
  ...
  { value: 'suv_mid', label: 'SUV — Mid', examples: 'Honda CR-V, Mazda CX-5, Toyota RAV4' },
    ...
    <div style={{ minHeight: '100vh', background: '#f3f4f6', fontFamily: 'Inter, sans-serif' }}>
      <nav style={{ background: '#1e3a5f', padding: '0 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <Link to="/hongkong" style={{ background: '#0d9488', color: 'white', padding: '6px 16px', borderRadius: 20, fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>🇭🇰 Hong Kong</Link>
        </div>
      </nav>

      <div style={{ background: 'linear-gradient(135deg, #0f2236 0%, #1a3a50 100%)', padding: '48px 32px 40px', textAlign: 'center' }}>
        <p style={{ color: '#5eead4', fontWeight: 700, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.12em', margin: '0 0 12px' }}>Hong Kong • Free Tool</p>
        <h1 style={{ color: 'white', fontSize: 38, fontWeight: 800, margin: '0 0 14px', lineHeight: 1.1 }}>Is your lease a fair deal?</h1>
        <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 16, maxWidth: 520, margin: '0 auto', lineHeight: 1.65 }}>Enter your lease terms and we’ll compare them against current Hong Kong market rates.</p>
      </div>
          ...
          <>
            <div style={cardStyle}>
              ...
              <div>
                ...
                <input style={{ ...inputStyle, borderColor: errors.email ? '#0d9488' : '#d1d5db' }} type="email" placeholder="you@example.com" value={form.email} onChange={e => set('email', e.target.value)} />
          ...
          <>
            <div style={{ ...cardStyle, borderColor: report.color, borderWidth: 2 }}>
              ...
              <div style={{ background: '#f9fafb', borderRadius: 8, padding: '16px 20px', marginBottom: 18 }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: '#1f2937' }}>Your lease: HKD {parseFloat(form.cost).toLocaleString()}/mo — <span style={{ color: report.color }}>{report.verdict}</span></div>
              </div>
                  ...
                  {report.inclusions.map(inc => (
                    <div key={inc.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: inc.included ? '#f0fdf4' : '#fef2f2', borderRadius: 6 }}>
                      ...
                      <span style={{ fontSize: 13, color: inc.included ? '#16a34a' : '#dc2626', fontWeight: 600 }}>
                        {inc.included ? `Included (~HKD ${inc.value.toLocaleString()}/mo value)` : `Not included — budget ~HKD ${inc.value.toLocaleString()}/mo extra`}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              {form.mileage && form.excess && (
                <div style={{ marginTop: 14, background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 8, padding: '12px 16px', fontSize: 13, color: '#92400e' }}>
                  <strong>Mileage note:</strong> At {parseInt(form.mileage).toLocaleString()} km/mo, if you exceed by 500 km you’ll pay ~HKD {Math.round(parseFloat(form.excess) * 500).toLocaleString()} extra that month.
                </div>
              )}
            </div>
